...
### ⚙️ [Processamento][cite: 1]
1. Inicializar a contagem da cena atual em 1 (`cenaAtual = 1`)[cite: 1].
2. **Estrutura de Repetição (`while` / `for`):** Enquanto `cenaAtual <= quantidadeCenas`, executar[cite: 1]:
   * Ler os dados de `temCrianca` e `precisaFogo` da cena[cite: 1].
   * **Estrutura de Decisão (`if / else`):**[cite: 1]
   ...
   * Incrementar o contador de cenas (`cenaAtual = cenaAtual + 1`) para evitar laço infinito[cite: 1].

---

### 📤 [Saída][cite: 1]
* Lista consolidada informando a decisão de produção e o método de execução recomendado para cada uma das cenas analisadas no episódio[cite: 1].
